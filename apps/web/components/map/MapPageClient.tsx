...
function MapPageInner() {
  ...
  return (
    <div className="relative flex h-screen w-full flex-col" style={{ background: "#000" }}>
      <header className="absolute top-0 left-0 right-0 z-20 flex h-14 items-center justify-between px-5"
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-[11px] text-zinc-500 uppercase tracking-widest">Global Crisis Monitor</span>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={handleRefresh}
            disabled={refreshing}
            className="flex items-center gap-1.5 rounded-md border border-zinc-700 bg-zinc-900/60 px-3 py-1.5 text-[11px] text-zinc-300 hover:bg-zinc-800 hover:text-white transition-colors disabled:opacity-50"
      ...
      {regions && regions.length > 0 && (
        <div className="absolute bottom-4 left-4 z-10 flex items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900/80 px-3 py-2 backdrop-blur-sm">
          <div className="h-2 w-2 rounded-full bg-red-500 animate-pulse" />
          <span className="text-[11px] text-zinc-400">
            <span className="font-semibold text-white">{regions.length}</span> active crisis regions
          </span>
        </div>
      )}
    </div>
